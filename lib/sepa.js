...
(function(exports) {
  ...
  SepaDocument.prototype = {
    ...
    /** Xml encoding */
    _xmlEncoding: DEFAULT_XML_ENCODING,
    ...
    toXML: function() {
      ...
      body.setAttribute(
        "xsi:schemaLocation",
        XSI_NS + this._painFormat + " " + this._painFormat + ".xsd"
      );
  ...
  SepaGroupHeader.prototype = {
    ...
    toXML: function(doc) {
      ...
      var grpHdr = doc.createElementNS(
  ...
  SepaPaymentInfo.prototype = {
    ...
    toXML: function(doc) {
      ...
      var r = createXMLHelper(doc, true, true);
      var pmtInf = doc.createElementNS(
        doc.documentElement.namespaceURI,
        "PmtInf"
      );
  ...
  SepaTransaction.prototype = {
    ...
    validate: function() {
      var pullFrom =
        this._type === TransactionTypes.Transfer ? "creditor" : "debtor";
    ...
    toXML: function(doc) {
      ...
      var pullFrom =
        this._type === TransactionTypes.Transfer ? "creditor" : "debtor";
      var recieverNodeName =
        this._type === TransactionTypes.Transfer ? "Cdtr" : "Dbtr";

      var n = createXMLHelper(doc, true, false);
      var o = createXMLHelper(doc, false, true);
      var r = createXMLHelper(doc, true, true);

      var txInf = doc.createElementNS(
        doc.documentElement.namespaceURI,
        this._type
      );
      ...
      if (this._type === TransactionTypes.DirectDebit) {
        r(txInf, "InstdAmt", this.amount.toFixed(2)).setAttribute(
          "Ccy",
          this.currency
        );
        ...
        r(txInf, "Amt", "InstdAmt", this.amount.toFixed(2)).setAttribute(
          "Ccy",
          this.currency
        );
      }
  ...
  function checksumIBAN(iban) {
    ...
    return (
      iban.substr(0, 2) + ("0" + (98 - mod)).substr(-2, 2) + iban.substr(4)
    );
  }
  ...
  function assert_sepa_id_set1(str, member) {
    if (str && !str.match(/([A-Za-z0-9]|[+|?|/|\-|:|(|)|.|,|'| ]){1,35}/)) {
      throw new Error(
        member +
          " doesn't match sepa id charset type 1 (found: " +
          '"' +
          str +
          '")'
      );
  ...
  function assert_sepa_id_set2(str, member) {
    if (str && !str.match(/([A-Za-z0-9]|[+|?|/|\-|:|(|)|.|,|']){1,35}/)) {
      throw new Error(
        member +
          " doesn't match sepa id charset type 2 (found: " +
          '"' +
          str +
          '")'
      );
